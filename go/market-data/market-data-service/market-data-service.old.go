...
const (
	SplunkServiceName = "OTEL_SERVICE_NAME"
) // will be used to grab the env variable with the Env to use
...
func (s *service) Subscribe(_ context.Context, r *api.MdsSubscribeRequest) (*model.Empty, error) {
	ctx, span := otel.Tracer("otp-tracer").Start(context.Background(), "Subscription")
	defer span.End()
	spanCtx := trace.SpanContextFromContext(ctx)
	logrus.WithFields(LogrusFields(spanCtx)).Info("Handling subscription")
...
func (s *service) Connect(request *api.MdsConnectRequest, stream api.MarketDataService_ConnectServer) error {
	ctx, span := otel.Tracer("otp-tracer").Start(context.Background(), "Connect")
	defer span.End()
	spanCtx := trace.SpanContextFromContext(ctx)

	subscriberId := request.GetSubscriberId()
	logrus.WithFields(LogrusFields(spanCtx)).Info("connect request received", "subscriberId", subscriberId)

	s.mutex.Lock()
	defer s.mutex.Unlock()
	if connection, ok := s.subscriberIdToConnection[subscriberId]; ok {
		logrus.WithFields(LogrusFields(spanCtx)).Info("connection already exists, closing existing connection", "subscriberId", subscriberId)
		connection.Close()
		connections.Dec()
	}
	...
	for {
		...
		case <-stream.Context().Done():
			logrus.WithFields(LogrusFields(spanCtx)).Info("connection closed", "subscriberId", subscriberId)
			return nil
		case quote := <-connection.Chan():
			if err := stream.Send(quote); err != nil {
				logrus.WithFields(LogrusFields(spanCtx)).Error("failed to send quote, closing connection", "subscriberId", subscriberId, "error", err)
...
func main() {
	...
	ctx := context.TODO()
	ctx, span := otel.Tracer("otp-tracer").Start(context.Background(), "Market-service-main")
	defer span.End()
	spanCtx := trace.SpanContextFromContext(ctx)
	...
	go func() {
		...
		if err != nil {
			logrus.WithFields(LogrusFields(spanCtx)).Error("failed to listen on metrics server port", "error", err)
		}
	}()
	...
	if err != nil {
		logrus.WithFields(LogrusFields(spanCtx)).Panicf("failed to get static data source:%v", err)
	}
	...
	if err != nil {
		logrus.WithFields(LogrusFields(spanCtx)).Panicf("Error watching pods: %v", err)
	}
	...
	go func() {
		for e := range pods.ResultChan() {
			...
			if err != nil {
				logrus.WithFields(LogrusFields(spanCtx)).Error("failed to get balancing stateful pod", "pod", pod, "error", err)
				continue
			}

			if e.Type == watch.Added {
				if err = cf.AddMarketDataGateway(marketDataService{bsp: bsp}); err != nil {
					logrus.WithFields(LogrusFields(spanCtx)).Error("failed to add new gateway", "balancingStatefulPod", bsp, "error", err)
				}
			}
		}
	}()
	...
	port := "50551"
	logrus.WithFields(LogrusFields(spanCtx)).Info("starting market data service", "port", port)
	lis, err := net.Listen("tcp", "0.0.0.0:"+port)
	if err != nil {
		logrus.WithFields(LogrusFields(spanCtx)).Panicf("Error while listening : %v", err)
	}
	...
	if err := s.Serve(lis); err != nil {
		logrus.WithFields(LogrusFields(spanCtx)).Panicf("Error while serving : %v", err)
	}
}
